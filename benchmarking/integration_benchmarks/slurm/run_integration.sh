#!/bin/bash
# Universal CARIBOU integration benchmark run launcher.
#
# Usage:
#   sbatch run_integration.sh --dataset aba_hippocampus --llm chatgpt --mode full_system
#   sbatch run_integration.sh --dataset aba_hippocampus --llm deepseek --mode single_agent
#
# All datasets are defined in  ../datasets/<id>/config.json  and  prompt.txt.
#
#SBATCH --job-name=caribou_integration
...
if [[ -z "$DATASET" || -z "$LLM_BACKEND" || -z "$MODE" ]]; then
    echo "Usage: sbatch run_integration.sh --dataset <id> --llm <chatgpt|deepseek|claude> --mode <full_system|full_system_no_mem|single_agent|one_shot>"
    exit 1
fi
...
if [[ ! -f "$CONFIG_PATH" ]]; then
    ...
    echo "Available datasets:"; ls "$INTBENCH_DIR/datasets/"; exit 1
fi
...
if command -v scontrol &>/dev/null && [[ -n "${SLURM_JOB_ID:-}" ]]; then
    scontrol update JobId="$SLURM_JOB_ID" JobName="intg_${DATASET}_${LLM_BACKEND}_${MODE}"
fi
...
START_TIME=$(date +%s)
echo "====================================================="
echo "  CARIBOU Integration Benchmark Run"
...
if [[ "$MODE" == "full_system" ]]; then
    ...
    python "$CARIBOU_ROOT/benchmarking/task_benchmarks/src/one_shot_runner.py" \
        ...
        --llm          "$LLM_BACKEND" \
        --prompt       "$PROMPT_PATH" \
        --benchmark-id "integration_${DATASET}" \
        2>&1 | tee "$RUN_DIR/run.log"
