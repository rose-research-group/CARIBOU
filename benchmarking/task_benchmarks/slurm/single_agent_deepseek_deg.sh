#!/bin/bash
#SBATCH --job-name=deg_single_agent_deepseek
...
mkdir -p "$LOG_DIR"
LOG_PATH="$LOG_DIR/single_agent_deepseek_deg_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}.log"
exec > "$LOG_PATH" 2>&1

# Configuration
BLUEPRINT_PATH="$ROOT_DIR/benchmarking/task_benchmarks/configs/deg_single_agent.json"
DATASET_PATH="$ROOT_DIR/dev/datasets/pbmc_1k_v2_v3_combined.h5ad"
OUTPUT_BASE="$ROOT_DIR/benchmarking/task_benchmarks/results/deg_task/single_agent"
