...
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/one_shot_claude_qc.sh"
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/one_shot_deepseek_qc.sh"

# Single-agent tests
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/single_agent_chatgpt_qc.sh"
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/single_agent_claude_qc.sh"
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/single_agent_deepseek_qc.sh"

# Full-system tests
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/full_system_chatgpt_qc.sh"
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/full_system_claude_qc.sh"
sbatch --chdir "$ROOT_DIR" "$SLURM_DIR/full_system_deepseek_qc.sh"
