#!/bin/bash
#SBATCH --job-name=batch_correction_single_agent_deepseek
...
mkdir -p "$LOG_DIR"
LOG_PATH="$LOG_DIR/single_agent_deepseek_batch_correction_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}.log"
exec > "$LOG_PATH" 2>&1

# Configuration
BLUEPRINT_PATH="$ROOT_DIR/benchmarking/task_benchmarks/configs/batch_correction_single_agent.json"
DATASET_PATH="$ROOT_DIR/dev/datasets/pbmc_1k_v2_v3_combined.h5ad"
OUTPUT_BASE="$ROOT_DIR/benchmarking/task_benchmarks/results/batch_correction_task/single_agent"
SANDBOX_BACKEND="singularity"
BENCHMARK_ID="batch_correction_benchmark"
LLM_BACKEND="deepseek"
NUM_TURNS=15
NUM_TRIALS=3

PROMPT_PATH="${PROMPT_PATH:-$ROOT_DIR/benchmarking/task_benchmarks/prompts/batch_correction_prompt.txt}"
...
for trial in $(seq 1 "$NUM_TRIALS"); do
    ...
    caribou run auto \
        ...
        --driver-agent "batch_correction_agent" \
        --output-dir "$RUN_DIR" \
