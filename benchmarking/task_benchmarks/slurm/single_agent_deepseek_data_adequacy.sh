#!/bin/bash
#SBATCH --job-name=data_adequacy_single_agent_deepseek
...
mkdir -p "$LOG_DIR"
LOG_PATH="$LOG_DIR/single_agent_deepseek_data_adequacy_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}.log"
exec > "$LOG_PATH" 2>&1

# Configuration
BLUEPRINT_PATH="$ROOT_DIR/benchmarking/task_benchmarks/configs/data_adequacy_single_agent.json"
DATASET_PATH="$ROOT_DIR/dev/datasets/pbmc_1k_v2_v3_combined.h5ad"
OUTPUT_BASE="$ROOT_DIR/benchmarking/task_benchmarks/results/data_adequacy_task/single_agent"
SANDBOX_BACKEND="singularity"
BENCHMARK_ID="data_adequacy_benchmark"
LLM_BACKEND="deepseek"
NUM_TURNS=6
NUM_TRIALS=3

PROMPT_PATH="${PROMPT_PATH:-$ROOT_DIR/benchmarking/task_benchmarks/prompts/data_adequacy_prompt.txt}"
...
for trial in $(seq 1 "$NUM_TRIALS"); do
    ...
    caribou run auto \
        ...
        --driver-agent "data_adequacy_agent" \
        --output-dir "$RUN_DIR" \
