#!/bin/bash
#SBATCH --job-name=doublet_single_agent_deepseek
...
mkdir -p "$LOG_DIR"
LOG_PATH="$LOG_DIR/single_agent_deepseek_doublet_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}.log"
exec > "$LOG_PATH" 2>&1
...
PROMPT_PATH="${PROMPT_PATH:-$ROOT_DIR/benchmarking/task_benchmarks/prompts/doublet_prompt.txt}"
...
for trial in $(seq 1 "$NUM_TRIALS"); do
    echo "================================================================================"
    echo "Starting Single Agent doublet Trial $trial of $NUM_TRIALS"
...
echo "All single-agent doublet trials complete"
